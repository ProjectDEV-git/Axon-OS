...
class TestGetGpuInfo:
    ...
    def test_amd_rocm_detected(self):
        with patch("shutil.which", side_effect=lambda x: "/usr/bin/rocm-smi" if x == "rocm-smi" else None):
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = MagicMock(
    ...
    def test_lspci_nvidia_fallback(self):
        with patch("shutil.which", return_value=None):
            with patch("subprocess.check_output", return_value="01:00.0 VGA compatible controller: NVIDIA Corporation"):
                gpu = get_gpu_info()
        assert gpu["vendor"] == "NVIDIA"
    ...
    def test_lspci_amd_fallback(self):
        with patch("shutil.which", return_value=None):
            with patch("subprocess.check_output", return_value="01:00.0 VGA: Advanced Micro Devices"):
                gpu = get_gpu_info()
        assert gpu["vendor"] == "AMD"
...
class TestProfileHardware:
    def test_returns_hardware_and_recommendations(self):
        with patch("services.axon_brain.hardware_profiler.get_system_ram", return_value=16.0):
            with patch("services.axon_brain.hardware_profiler.get_gpu_info", return_value={
                "vendor": "NVIDIA", "model": "RTX 3060", "vram": 12.0, "status": "detected"
            }):
    ...
    def test_high_vram_nvidia_recommendations(self):
        with patch("services.axon_brain.hardware_profiler.get_system_ram", return_value=32.0):
            with patch("services.axon_brain.hardware_profiler.get_gpu_info", return_value={
                "vendor": "NVIDIA", "model": "RTX 4090", "vram": 24.0, "status": "detected"
            }):
                profile = profile_hardware()
    ...
    def test_low_vram_recommendations(self):
        with patch("services.axon_brain.hardware_profiler.get_system_ram", return_value=8.0):
            with patch("services.axon_brain.hardware_profiler.get_gpu_info", return_value={
                "vendor": "NVIDIA", "model": "GT 1030", "vram": 2.0, "status": "detected"
            }):
                profile = profile_hardware()
    ...
    def test_cpu_with_high_ram(self):
        with patch("services.axon_brain.hardware_profiler.get_system_ram", return_value=32.0):
            with patch("services.axon_brain.hardware_profiler.get_gpu_info", return_value={
                "vendor": "CPU", "model": "Generic", "vram": 0.0, "status": "fallback"
            }):
                profile = profile_hardware()
