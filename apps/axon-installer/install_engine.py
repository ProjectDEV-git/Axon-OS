...
def format_and_mount(esp: str, root: str, esp_is_new: bool) -> None:
...
def unmount_all() -> None:
    for mp in (f"{TARGET}/dev/pts", f"{TARGET}/dev", f"{TARGET}/proc",
               f"{TARGET}/sys", f"{TARGET}/boot/efi", TARGET):
        run(["umount", "-lf", mp], check=False)


def write_fstab(esp: str, root: str) -> None:
    root_uuid = run(["blkid", "-s", "UUID", "-o", "value", root], capture=True).strip()
    lines = [
        "# /etc/fstab — generated by the Axon OS installer",
...
def install(cfg: dict) -> None:
    ...
    format_and_mount(esp, root, esp_is_new)
